...
class Classifier(Network):
    ...
    def backward(self, dout):
        ...
        X, logits, z = self.cache

        # Derivative of sigmoid: sigmoid'(x) = sigmoid(x) * (1 - sigmoid(x))
        d_sigmoid = z * (1 - z)

        # Apply chain rule: dL/dlogits = dL/dz * dz/dlogits
        d_logits = dout * d_sigmoid

        # Chain rule for affine: dL/dW = X^T * dL/dlogits
        dW = X.T @ d_logits
